...
export class AddDealDialogBoxComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    if (this.isActiveModal.data.mode === "editing") {
      ...
      this.tempDealImage = this.deals.deal_image;
    }

    this.dealForm = new FormGroup({
      name: new FormControl(this.deals ? this.deals.name : null, [
        Validators.required
      ]),
      price: new FormControl(this.deals ? this.deals.price : null, [
        Validators.required
      ]),
      ...
      startTime: new FormControl(
        this.deals ? new Date(this.deals.startTime) : startTime
      ),
      endTime: new FormControl(
        this.deals ? new Date(this.deals.endTime) : startTime
      )
    });
  ...
  mapDealItems() {
    this.dealItemsList = this.itemsList
      .filter(i => !i.editing)
      .map(data => {
        if (data.item) {
          return {
